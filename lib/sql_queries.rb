...
"SELECT projects.title, SUM(amount)-projects.funding_goal FROM projects
INNER JOIN pledges ON projects.id = pledges.project_id
GROUP BY pledges.project_id HAVING SUM(amount) >= projects.funding_goal"
end

def selects_user_names_and_amounts_of_all_pledges_grouped_by_name_then_orders_them_by_the_amount_and_users_name
"SELECT users.name, SUM(amount) FROM users
INNER JOIN pledges ON users.id = pledges.user_id
GROUP BY users.name ORDER BY SUM(amount), users.name"
end
